# pdp_plan_common_review 通用审核

## 基本信息

| 字段 | 值 |
|------|-----|
| 流程Key | `pdp_plan_common_review` |
| 计划类型 | 【流程】通用审核 |
| 所属模块 | PDP |
| 表单类型 | 自定义表单 |
| businessKey | `planId` |
...
复用 `AbstractPlanProcessStatusListener` 基类逻辑，监听 key = `pdp_plan_common_review`。

## 设计要点

- 节点结构与 `pdp_plan_common`（计划通用流程）完全一致
- `pdp_plan_common` 是底层通用流程，`pdp_plan_common_review` 是计划类型系统中的"通用审核"入口
